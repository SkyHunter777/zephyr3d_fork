  ...
  implements Clonable<PBRBluePrintMaterial>
{
  ...
  vertexShader(scope: PBFunctionScope) {
    ...
    if (vertexOut.Tangent) {
      ...
      scope.$l.oTangent = tangent.getTypeName() === 'vec4' ? tangent : pb.vec4(this.toVec3(scope as unknown as PBInsideFunctionScope, tangent), 1);
  ...
  calculateNormalAndTBN(
    ...
    return pb.getGlobalScope()[funcName](
      worldPos,
      (worldNormal ?? scope.zVertexNormal) as PBShaderExp,
      (worldTangent ?? scope.zVertexTangent) as PBShaderExp,
      (worldBinormal ?? scope.zVertexBinormal) as PBShaderExp,
      outputNormal !== undefined ? this.toVec3(scope, outputNormal) : pb.vec3(0, 0, 1),
      outputTangent !== undefined ? this.toVec3(scope, outputTangent) : pb.vec3(0)
    ) as PBShaderExp;
  }
  ...
  private getBlueprintOutputMap(scope: PBInsideFunctionScope): BlueprintOutputMap {
    ...
    const result = pb.getGlobalScope()[funcName](
      this.getBlueprintWorldPos(scope),
      this.getBlueprintFragmentVertexColor(scope),
      this.getBlueprintFragmentVertexUV(scope),
      this.getBlueprintFragmentVertexNormal(scope),
      this.getBlueprintFragmentVertexTangent(scope),
      this.getBlueprintFragmentVertexBinormal(scope)
    ) as PBShaderExp;
