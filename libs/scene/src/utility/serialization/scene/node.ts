      ...
      return defineProps([
        ...
        },
        {
          ...
          getDefaultValue(this: SceneNode) {
            ...
            const rigs = new Set(animationSet.rigs.map((value) => value.get()));
            for (const binding of animationSet.skeletons) {
              const rig = binding.get()?.rig;
              if (rig) {
                rigs.add(rig);
          ...
          get(this: SceneNode, value) {
            const animationSet = this.animationSet;
            const springModifiers: SpringModifier[] = [];
            const rigs = new Set(animationSet.rigs.map((rigRef) => rigRef.get()));
            for (const binding of animationSet.skeletons) {
              const rig = binding.get()?.rig;
              if (rig) {
                rigs.add(rig);
          ...
          set(this: SceneNode, value) {
            ...
            for (const modifier of value.object) {
              ...
              const system = modifier.springSystem as unknown as {
                chain?: { particles?: { node?: SceneNode | null }[] };
                chains?: { particles?: { node?: SceneNode | null }[] }[];
              };
              const chains = Array.isArray(system.chains)
                ...
                : system.chain
                  ? [system.chain]
                  : [];
              const particleNodes = chains.flatMap((chain) =>
                (chain.particles ?? []).map((particle) => particle.node).filter((node) => !!node)
              );
              for (const rigRef of animationSet.rigs) {
                const rig = rigRef.get()!;
                if (particleNodes.length > 0 && particleNodes.every((node) => rig.joints.includes(node!))) {
          ...
          delete(this: SceneNode, index) {
            let current = 0;
            for (const rigRef of this.animationSet.rigs) {
              const modifiers = rigRef.get()!.modifiers;
