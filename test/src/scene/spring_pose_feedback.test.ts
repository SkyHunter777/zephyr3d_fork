...
describe.each(systemFactories)('%s input pose tracking', (_name, createSystem) => {
  test('keeps the authored target pose stable across unanimated simulation frames', () => {
  ...
  test('does not feed the previous spring output back into an unanimated target pose', () => {
    const fixture = createChainFixture(createSystem);
    try {
  ...
  test('keeps an external parent pose while restoring unanimated spring joint rotations', () => {
    const fixture = createChainFixture(createSystem);
    try {
      fixture.chain.particles[1].position.setXYZ(1, -1, 0);
      fixture.chain.particles[2].position.setXYZ(2, -2, 0);
  ...
  test('accepts a new upstream rotation written directly to a spring joint', () => {
    const fixture = createChainFixture(createSystem);
    try {
      fixture.chain.particles[1].position.setXYZ(1, -1, 0);
      fixture.chain.particles[2].position.setXYZ(2, -2, 0);
      fixture.system.applyToNodes(1);
      const upstreamRotation = Quaternion.fromAxisAngle(Vector3.axisPZ(), Math.PI / 2);
      fixture.root.rotation.set(upstreamRotation);

      fixture.system.update(1 / 60);

      expect(fixture.chain.particles[1].animPosition.x).toBeCloseTo(0);
      expect(fixture.chain.particles[1].animPosition.y).toBeCloseTo(1);
      expect(fixture.root.rotation.equalsTo(upstreamRotation)).toBe(true);
    } finally {
      fixture.scene.dispose();
    }
  });
